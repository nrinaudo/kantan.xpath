...
/** Describes how to retry failed request for [[RemoteXmlSource]].
  ...
  *
  * @param max    maximum number of attempts that can be made for a given request.
  * @param delay  delay, in milliseconds, between each attempt.
  * @param factor function that calculates the delay factor based on the number of requests already attempted.
